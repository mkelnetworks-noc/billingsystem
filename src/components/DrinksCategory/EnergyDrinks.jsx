...
const EnergyDrinks = ({ addToCart }) => {
  ...
  useEffect(() => {
    ...
    fetch('/drinks.json')
      ...
      .then((data) => {
        setEnergyDrinks(data.energyDrinks);
        setQuantities(data.energyDrinks.map(() => 1));
      })
      .catch((error) => {
        console.error('Error fetching the energy drinks data:', error);
      });
  }, []);
    ...
    <section id="energy-drinks" className="p-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {energyDrinks.length > 0 ? (
          ...
          <p className="text-gray-600">No energy drinks available at the moment.</p>
        )}
      </div>
    </section>
  );
