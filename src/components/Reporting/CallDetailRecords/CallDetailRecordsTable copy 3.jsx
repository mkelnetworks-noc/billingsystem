...
const CallDetailRecordsTable = () => {
  ...
  const fetchFilteredData = async () => {
    ...
    try {
      ...
      // Make GET request to fetch filtered records with pagination
      const response = await axios.get("http://localhost:3001/records/filtered-records", { params });

      const { records, totalRecords } = response.data; // Assuming totalRecords is returned by API

      setFilteredData(records);

      // Calculate total pages based on total records and page size
      setTotalPages(Math.ceil(totalRecords / 500)); // Adjust page size if necessary
    } catch (err) {
  ...
  // Effect hook to fetch filtered data when page changes
  useEffect(() => {
    if (currentPage > 1) {
      fetchFilteredData();
    }
